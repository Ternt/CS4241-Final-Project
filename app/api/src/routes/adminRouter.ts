...
import { CourseType, RawCourseType, SectionType, SubjectType } from "@repo/app-commons/types/types";
import { determineSubjectCategory } from "../util/data-parsing.util";
...
router.get("/populate_database", async (req: Request, res: Response) => {
  try {
    ...
    response.data.Report_Entry.map((courseData: RawCourseType) => {
      ...
      if (!subjects[code]) {
        subjects[code] = {
          ...
          category: determineSubjectCategory(subject, courseData.Course_Section_Owner),
        };
      }
    });
    ...
    console.error("[ERROR] Cannot get public course listing: " + error);
